...
(function(modules) {
	...
	checkDeferredModules();
		...
		gameObject.start = function(data) {
			...
			if (gameObject.replaying) {
				...
				if (settings.showChat) {
					gameObject.events.$emit('chat-visible', { visible: true });
				}
		...
		gameObject.stop = function() {
			...
			const {scene:gameScene} = gameObject;        
			if (gameScene) {
				gameScene.destroyBackgroundImage(false);
				gameScene.uninstallMassTextFont();
				gameScene.container?.destroy(true);
		...
		gameObject.updateCamera = function(server) {
			const {scene:gameScene, camera} = gameObject;
			...
			// camera position
			const cx = gameScene.container.pivot.x = lerp(camera.ox, camera.nx, movementSpeed);
			const cy = gameScene.container.pivot.y = lerp(camera.oy, camera.ny, movementSpeed);
	
			const scale = lerp(camera.oz, camera.nz, zoomSpeed);
			gameScene.container.scale.set(scale);
	...
	const addOrUpdate = (data, tick) => {
		...
		if (node) {
			...
			const {scene:gameScene} = gameObject;
			gameScene[!!(flags & 1) ? 'addFood' : 'addCell'](node.sprite);
	...
	const parseMessage = (view) => {
		...
		switch (op) {
			...
			case 25: {
				gameObject.events.$emit('update-cautions', {
					custom: buffer.readString16()
				});
				return;
			}
	...
	var Ot = Dt.exports,
		Nt = function() {
			...
			return s("div", {
                ...
                style: e.visibleToast ? {} : { height: '200px' }
